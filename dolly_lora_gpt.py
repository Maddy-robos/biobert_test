# -*- coding: utf-8 -*-
"""Dolly - LoRa - GPT-J-6b + Cleaned Alpaca Dataset.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1p1Gt8-YMCn6rAbdvUu1dpryMQN6g1fxs

# Dolly - LoRa - GPT-J-6b 

About Dolly - https://www.databricks.com/blog/2023/03/24/hello-dolly-democratizing-magic-chatgpt-open-models.html

Cleaned Alpaca Dataset - https://github.com/gururise/AlpacaDataCleaned
"""

from transformers import AutoTokenizer, AutoModelForCausalLM

tokenizer = AutoTokenizer.from_pretrained("EleutherAI/gpt-j-6B", load_in_8bit_fp32_cpu_offload=True)

tokenizer.pad_token = tokenizer.eos_token
tokenizer.pad_token_id = tokenizer.eos_token_id

from peft import PeftModel
from transformers import AutoTokenizer, GPTJForCausalLM, GenerationConfig

device = 'cpu'
model = GPTJForCausalLM.from_pretrained(
    "EleutherAI/gpt-j-6B",
    device_map={"cuda:0": device, "cuda:1": device} if "cuda" in device else None,
)
model = PeftModel.from_pretrained(model, "samwit/dolly-lora")

PROMPT = """Below is an instruction that describes a task. Write a response that appropriately completes the request.
### Instruction:
What are the differences between alpacas and sheep?
### Response:"""

def dolly_generate(text):
    inputs = tokenizer(
        PROMPT,
        return_tensors="pt",
    )
    input_ids = inputs["input_ids"].cuda()

    generation_config = GenerationConfig(
        temperature=0.6,
        top_p=0.95,
        repetition_penalty=1.2,
    )

    print("Generating...")
    generation_output = model.generate(
        input_ids=input_ids,
        generation_config=generation_config,
        return_dict_in_generate=True,
        output_scores=True,
        max_new_tokens=128,
        pad_token_id = 0,
        eos_token_id = 50256
    )

    for s in generation_output.sequences:
        print(tokenizer.decode(s))

# Commented out IPython magic to ensure Python compatibility.
# %%time
# dolly_generate(PROMPT)

PROMPT = """Below is an instruction that describes a task. Write a response that appropriately completes the request.
### Instruction:
Write me a python function that checks if a number is prime.
### Response:"""

dolly_generate(PROMPT)

PROMPT = """Below is an instruction that describes a task. Write a response that appropriately completes the request.
### Instruction:
Write a SOAP encounter for a patient with diabetes.
### Response:"""

dolly_generate(PROMPT)